 ...
 * @author Luke Bingham
 */
public final class LocaleChatHandler extends LocaleHandler<String, String> {

    private static final Pattern JSON_PATTERN = Pattern.compile("\\{t(?:\\=|\\\\u003d)[^\"\\}]+\\}");

    public LocaleChatHandler(JavaPlugin plugin) {
        super(plugin);

        ProtocolLibrary.getProtocolManager().addPacketListener(new PacketAdapter(
                   new PacketAdapter.AdapterParameteters()
                           .plugin(plugin).optionAsync()
                           .types(WrapperPlayServerChat.TYPE)) {
                       ...
                       public void onPacketSending(PacketEvent event) {
                           ...
                           String json = packet.getMessage().getJson();
